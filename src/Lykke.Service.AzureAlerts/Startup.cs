...
using Lykke.Service.AzureAlerts.Modules;
...
namespace Lykke.Service.AzureAlerts
{
    public class Startup
    {
        ...
        public IServiceProvider ConfigureServices(IServiceCollection services)
        {
            ...
            {
                options.DefaultLykkeConfiguration("v1", "AzureAlerts API");
            });
            ...
            builder.RegisterModule(new ServiceModule(appSettings.AzureAlertsService, log));
        ...
        public void Configure(IApplicationBuilder app, IHostingEnvironment env, IApplicationLifetime appLifetime)
        {
            ...
            app.UseLykkeMiddleware("AzureAlerts", ex => new {Message = "Technical problem"});
        ...
        private static ILog CreateLogWithSlack(IServiceCollection services, AppSettings settings)
        {
            ...
            var dbLogConnectionString = settings.AzureAlertsService.Db.LogsConnString;

            // Creating azure storage logger, which logs own messages to concole log
            if (!string.IsNullOrEmpty(dbLogConnectionString) && !(dbLogConnectionString.StartsWith("${") && dbLogConnectionString.EndsWith("}")))
            {
                const string appName = "Lykke.Service.AzureAlerts";

                var persistenceManager = new LykkeLogToAzureStoragePersistenceManager(
                    appName,
                    AzureTableStorage<LogEntity>.Create(() => dbLogConnectionString, "AzureAlertsLog", consoleLogger),
                    consoleLogger);
